...
#+begin_src typst :file diagrams/png/ndt-algorithm.png :results file graphics :exports results
...
#diagram(
  ...
  node((0,0), [*Input*: Scan + Initial Guess], fill: rgb("#e3f2fd"), name: <input>),
  edge("-|>"),
  node((0,1), [Transform Scan Points], fill: rgb("#fff3e0"), name: <transform>),
  edge("-|>"),
  node((0,2), [Find Nearby Voxels], fill: rgb("#fff3e0")),
  edge("-|>"),
  node((0,3), [Compute Score, Gradient, Hessian], fill: rgb("#ffebee")),
  edge("-|>"),
  node((0,4), [Newton Step: δ = −H⁻¹g], fill: rgb("#fff3e0")),
  edge("-|>"),
  node((0,5), [Line Search], fill: rgb("#fff3e0")),
  edge("-|>"),
  node((0,6), [Converged?], fill: rgb("#e8f5e9"), shape: fletcher.shapes.diamond, name: <converge>),
  edge(<converge>, (1,6), "-|>", [Yes]),
  node((1,6), [*Output*: Final Pose], fill: rgb("#c8e6c9"), name: <output>),
  edge(<converge>, <transform>, "-|>", [No], bend: -90deg),
)
#+end_src

#+RESULTS:
[[file:diagrams/png/ndt-algorithm.png]]
...
#+begin_src typst :file diagrams/png/ros-processing-flow.png :results file graphics :exports results
#import "@preview/fletcher:0.5.8" as fletcher: diagram, node, edge

#let marker(n) = box(fill: white, stroke: 0.5pt, inset: 2pt, radius: 2pt)[#text(size: 7pt)[#n]]

#grid(columns: (auto, 1em, auto), gutter: 0.5em,
  // Diagram
  diagram(
    node-stroke: 1pt,
    node-corner-radius: 4pt,
    ...
    // NDT in center, others around it
    node((1,1), [*NDT*], fill: rgb("#e8f5e9"), name: <ndt>),
    node((0,0), [*EKF*], fill: rgb("#e3f2fd"), name: <ekf>),
    node((2,0), [*LiDAR*], fill: rgb("#fff3e0"), name: <lidar>),
    node((0,2), [*Map*], fill: rgb("#f3e5f5"), name: <map>),
    node((2,2), [*TF*], fill: rgb("#ffebee"), name: <tf>),

    // Message flow with markers on edges
    edge(<ekf>, <ndt>, "->", marker[1], label-sep: 0pt),
    edge(<lidar>, <ndt>, "->", marker[2], label-sep: 0pt),
    edge(<ndt>, <map>, "->", marker[3], label-sep: 0pt),
    edge(<map>, <ndt>, "->", marker[4], label-sep: 0pt, bend: 30deg),
    edge(<ndt>, <ekf>, "->", marker[5], label-sep: 0pt, bend: 30deg),
    edge(<ndt>, <tf>, "->", marker[6], label-sep: 0pt),
  ),
  [],
  // Legend
  align(left + horizon)[
    #set text(size: 9pt)
    #table(stroke: none, columns: 2, inset: 3pt, align: left,
      marker[1], [EKF → NDT: Initial pose],
      marker[2], [LiDAR → NDT: Point cloud],
      marker[3], [NDT → Map: Request tiles],
      marker[4], [Map → NDT: Map data],
      marker[5], [NDT → EKF: Aligned pose],
      marker[6], [NDT → TF: Transform],
    )
  ]
)
#+end_src
...
#+RESULTS:
[[file:diagrams/png/ros-processing-flow.png]]
...
#+begin_src typst :file diagrams/png/column-major-layout.png :results file graphics :exports results
// Color palette for gradient components
#let colors = (
  rgb("#e3f2fd"), rgb("#bbdefb"), rgb("#90caf9"),
  rgb("#64b5f6"), rgb("#42a5f5"), rgb("#2196f3")
)
#let cell(c, t) = box(fill: c, inset: 4pt, width: 2.2em, height: 1.5em)[#align(center + horizon)[#text(size: 8pt)[#t]]]

#grid(columns: 2, gutter: 2em,
  // Row-Major layout
  stack(dir: ttb, spacing: 0.5em,
    [*Row-Major* (scattered reduction)],
    table(
      columns: 7, stroke: 0.5pt, inset: 0pt, align: center,
      table.header([], [g0], [g1], [g2], [g3], [g4], [g5]),
      [P0], cell(colors.at(0), [0]), cell(colors.at(1), [1]), cell(colors.at(2), [2]), cell(colors.at(3), [3]), cell(colors.at(4), [4]), cell(colors.at(5), [5]),
      [P1], cell(colors.at(0), [6]), cell(colors.at(1), [7]), cell(colors.at(2), [8]), cell(colors.at(3), [9]), cell(colors.at(4), [10]), cell(colors.at(5), [11]),
      [P2], cell(colors.at(0), [12]), cell(colors.at(1), [13]), cell(colors.at(2), [14]), cell(colors.at(3), [15]), cell(colors.at(4), [16]), cell(colors.at(5), [17]),
    ),
    text(size: 9pt)[Sum g0: indices 0, 6, 12... #text(fill: red)[(scattered)]],
  ),
  // Column-Major layout
  stack(dir: ttb, spacing: 0.5em,
    [*Column-Major* (contiguous reduction)],
    table(
      columns: 4, stroke: 0.5pt, inset: 0pt, align: center,
      table.header([], [P0], [P1], [P2]),
      [g0], cell(colors.at(0), [0]), cell(colors.at(0), [1]), cell(colors.at(0), [2]),
      [g1], cell(colors.at(1), [3]), cell(colors.at(1), [4]), cell(colors.at(1), [5]),
      [g2], cell(colors.at(2), [6]), cell(colors.at(2), [7]), cell(colors.at(2), [8]),
      [g3], cell(colors.at(3), [9]), cell(colors.at(3), [10]), cell(colors.at(3), [11]),
      [g4], cell(colors.at(4), [12]), cell(colors.at(4), [13]), cell(colors.at(4), [14]),
      [g5], cell(colors.at(5), [15]), cell(colors.at(5), [16]), cell(colors.at(5), [17]),
    ),
    text(size: 9pt)[Sum g0: indices 0, 1, 2 #text(fill: green)[(contiguous)]],
  ),
)
#+end_src

#+RESULTS:
[[file:diagrams/png/column-major-layout.png]]
...
#+begin_src typst :file diagrams/png/newton-solve.png :results file graphics :exports results
#let box-style = (stroke: 1pt + rgb("#333"), radius: 4pt, inset: 0.6em)
#let cpu-fill = rgb("#fff3e0")
#let gpu-fill = rgb("#e8f5e9")

#grid(columns: 2, gutter: 1.5em,
  // Autoware CPU solve
  box(..box-style, fill: cpu-fill)[
    *Autoware (CPU solve)*
    #v(0.3em)
    #set text(size: 9pt)
    1. Download H (36 floats)\
    2. Download g (6 floats)\
    3. Eigen SVD on CPU\
    4. Upload Δp (6 floats)\
    #v(0.3em)
    Latency: *~100μs*
  ],
  // CUDA GPU solve
  box(..box-style, fill: gpu-fill)[
    *CUDA (GPU solve)*
    #v(0.3em)
    #set text(size: 9pt)
    1. Jacobi SVD in kernel\
    #h(0.8em)#text(size: 8pt)[(6×6 is tiny)]\
    2. Compute Δp on GPU\
    3. Update pose on GPU\
    4. No CPU roundtrip\
    #v(0.3em)
    Latency: *~10μs*
  ]
...
#+RESULTS:
[[file:diagrams/png/newton-solve.png]]
...
#+begin_src typst :file diagrams/png/voxel-grid-pipeline.png :results file graphics :exports results
#import "@preview/fletcher:0.5.8" as fletcher: diagram, node, edge
...
#diagram(
  ...
  node((0,0), [*Points*\ N × 3], fill: rgb("#e3f2fd")),
  edge("-|>"),
  node((1,0), [*Morton*\ Encode], fill: rgb("#e3f2fd")),
  edge("-|>"),
  node((2,0), [*Radix*\ Sort], fill: rgb("#fff3e0")),
  edge("-|>"),
  node((3,0), [*Segment*\ Detect], fill: rgb("#fff3e0")),
  edge("-|>"),
  node((4,0), [*Compute*\ Stats], fill: rgb("#e3f2fd")),

  // Labels below
  node((1,1), text(size: 8pt)[CubeCL], stroke: none),
  node((2,1), text(size: 8pt)[CUB], stroke: none),
  node((3,1), text(size: 8pt)[CUB], stroke: none),
  node((4,1), text(size: 8pt)[CubeCL], stroke: none),

  // Output
  edge((4,0), (4,2), "-|>"),
  node((4,2), [*Output:* V voxels\ mean, inv_cov, count], fill: rgb("#e8f5e9")),
)
#+end_src

#+RESULTS:
[[file:diagrams/png/voxel-grid-pipeline.png]]
...
#+begin_src typst :file diagrams/png/batched-initial-pose.png :results file graphics :exports results
#let box-style = (stroke: 1pt + rgb("#333"), radius: 4pt, inset: 0.6em)
#let slow-fill = rgb("#ffebee")
#let fast-fill = rgb("#e8f5e9")

#grid(columns: 2, gutter: 1.5em,
  // Sequential Autoware
  box(..box-style, fill: slow-fill)[
    *Sequential (Autoware)*
    #v(0.3em)
    #set text(size: 9pt)
    *For each particle:*\
    #h(0.5em)1. Set pose\
    #h(0.5em)2. Full NDT alignment\
    #h(0.5em)3. Record score\
    #v(0.3em)
    Time: N × 3-5ms\
    #h(2em)= *300-500ms*
  ],
  // Batched CUDA
  box(..box-style, fill: fast-fill)[
    *Batched (CUDA)*
    #v(0.3em)
    #set text(size: 9pt)
    Upload all particles\
    (100-200 poses)\
    #v(0.3em)
    *Single batched kernel:*\
    #h(0.5em)• All transforms\
    #h(0.5em)• All searches\
    #h(0.5em)• All scores\
    #v(0.3em)
    Time: *~20-50ms*
  ]
...
#+RESULTS:
[[file:diagrams/png/batched-initial-pose.png]]
...
#+begin_src typst :file diagrams/png/full-gpu-pipeline.png :results file graphics :exports results
#import "@preview/fletcher:0.5.8" as fletcher: diagram, node, edge
...
#diagram(
  ...
  // === Section 1: Per-Alignment Setup ===
  node(enclose: ((0,0), (2,0)), fill: rgb("#e3f2fd"), stroke: 1.5pt, inset: 12pt, name: <setup>),
  node((1,-0.6), [*Per-Alignment Setup (CPU → GPU)*], stroke: none),
  node((0,0), [*Source Points*\ ~3 KB], fill: white),
  node((1,0), [*Voxel Means*\ ~36 KB], fill: white),
  node((2,0), [*Voxel InvCov*\ ~108 KB], fill: white),

  // Arrow to iteration section
  edge((1,0.7), (1,1.3), "-|>"),

  // === Section 2: Per-Iteration Loop ===
  node(enclose: ((0,2), (3,3)), fill: rgb("#f3e5f5"), stroke: 1.5pt, inset: 12pt, name: <iter>),
  node((1.5,1.4), [*Per-Iteration (All GPU)*], stroke: none),

  // Forward pipeline
  node((0,2), [Transform], fill: white, name: <xform>),
  edge("-|>"),
  node((1,2), [Search], fill: white),
  edge("-|>"),
  node((2,2), [Derivatives], fill: white),
  edge("-|>"),
  node((3,2), [Reduce], fill: white, name: <reduce>),

  // Feedback path
  edge(<reduce>, (3,3), "-|>"),
  node((3,3), [Newton\ Solve], fill: white, name: <newton>),
  edge("-|>"),
  node((2,3), [Update\ Pose], fill: white),
  edge("->"),
  node((1,3), [Converged?], fill: white, shape: fletcher.shapes.diamond, name: <conv>),
  edge(<conv>, <xform>, "->", [No], label-side: right, bend: -50deg),

  // Exit path
  edge(<conv>, (1,4), "-|>", [Yes], label-side: right),

  // === Section 3: Download Result ===
  node((1,4.5), [*Download Result*\ Pose + Score + Hessian\ (172 bytes)], fill: rgb("#e8f5e9"), name: <result>),
)
#+end_src

#+RESULTS:
[[file:diagrams/png/full-gpu-pipeline.png]]
